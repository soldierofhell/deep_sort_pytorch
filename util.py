...
def draw_offline(img_dir, track_json, sequence_json, config_yml):
    ...
    if config['output']['video']:
        vw = cv2.VideoWriter(config['output']['video'], cv2.VideoWriter_fourcc(*'MJPG'), int(config['output']['fps']), (config['output']['width'], config['output']['height'])) 
    
    for idx, img_file in enumerate(sorted(os.listdir(img_dir))):       
 
        img = cv2.imread(os.path.join(img_dir, img_file))
        tracks = track_list[idx][str(idx)] # track_dict[img_file] # track_dict = {'img_file': []}
